...
my $size=int($number/$ARGV[2]/1024/1024)+1;
$size=$size."m";
my $sam_file_name=(split /\//,$sam_file)[-1];
$sam_file_name=$read_count_outfile."/".$sam_file_name;
...
sub read_count
{
    ...
    open OUT,">$canshu[0]_read_count4.txt";
    foreach my $junction(keys %long_junction){
        ...
        if (exists $types_count{$junction}) {
            ...
            foreach my $read_start(@keys){
                $type2+=$types_count{$junction}{$read_start};
            }
        }
        print OUT "$junction\t$type1\t$type2\t$read_positions\n"
    }
    close OUT;
}        
my $thred=$ARGV[2];
my @thread_all=qw /1...$thred/;
for(my $i=0;$i<$thred;$i++ ){
    my $num;
    if ($i<10) {
        $num="00".$i;
    }else{
...
foreach my $file (@dir){
    ...
    if ($file=~m/$query[0-9]+_read_count4\.txt/gi) {
        open(IN,"$dir/$file") or die "";
        while (<IN>) {
            chomp (my $line=$_);
            my ($junction,$type1,$type2,$read_positions)=split /\t/,$line;
...
for (my $i=0;$i<$thred;$i++){
    ...
    my $file2="$sam_file_name$j"."_read_count4.txt";
    `rm $file`;
    `rm $file2`;
}
my %read_nums;
...
my $out_result_pir_temp=$SRR_acc."_psi_all\.txt";
